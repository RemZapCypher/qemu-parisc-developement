...
/* Additional constants from reference material */
...
/* Function prototypes for implemented functions */
...
#undef CASE_GET_REG32
    qemu_log_mask(LOG_UNIMP, "NCR710: read unknown register %02X\n", offset);
    return 0;
}
...
#define CASE_SET_REG32(name, addr) \
    ...
    case NCR710_SBCL_REG: /* SBCL */
        ...
        break;
    case NCR710_DSTAT_REG: case NCR710_SSTAT0_REG: case NCR710_SSTAT1_REG: case NCR710_SSTAT2_REG:
...
static void ncr710_device_reset(DeviceState *dev)
{
    ...
    /* Use the comprehensive soft reset from reference material */
    ncr710_soft_reset(s);

    /* Re-initialize the queue since soft_reset asserts it's empty */
    QTAILQ_INIT(&s->queue);

    /* Initialize additional state not in soft_reset */
    s->scripts.running = false;
    s->scripts.connected = false;
    s->scripts.initiator = false;
    s->scripts.phase = SCSI_PHASE_DATA_OUT;
...
{
    SysBusNCR710State *sysbus_s;
    NCR710State *s;

    /* Stop submitting new requests during drain */
    if (!bus) {
        qemu_log("NCR710: drained_begin called with NULL bus\n");
        return;
    }

    /* Verify bus structure integrity */
    if (!bus->info) {
        qemu_log("NCR710: drained_begin: bus->info is NULL! bus=%p\n", bus);
        return;
    }

    qemu_log("NCR710: drained_begin: bus=%p bus->info=%p\n", bus, bus->info);

    /* Get NCR710 state from the bus */
    sysbus_s = container_of(bus, SysBusNCR710State, ncr710.bus);
    if (!sysbus_s) {
        qemu_log("NCR710: drained_begin: failed to get sysbus state from bus\n");
        return;
    }

    s = &sysbus_s->ncr710;
    if (!s) {
        qemu_log("NCR710: drained_begin: failed to get ncr710 state\n");
        return;
    }

    qemu_log("NCR710: drained_begin: stopping SCRIPTS and suspending operations\n");
...
static void ncr710_drained_end(SCSIBus *bus)
{
    ...
    if (!bus) {
        qemu_log("NCR710: drained_end called with NULL bus\n");
        return;
    }
    ...
    sysbus_s = container_of(bus, SysBusNCR710State, ncr710.bus);
    if (!sysbus_s) {
        qemu_log("NCR710: drained_end: failed to get sysbus state from bus\n");
        return;
    }

    s = &sysbus_s->ncr710;
    if (!s) {
        qemu_log("NCR710: drained_end: failed to get ncr710 state\n");
        return;
    }

    qemu_log("NCR710: drained_end: resuming operations\n");
